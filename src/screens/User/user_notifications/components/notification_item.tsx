...
const buildNotificationStyle = (notificationTypeId: string, issuerName: string) => {
...
type NotificationItemProps = { id: string, notificationTypeId: string, issuerName: string, tripId: string, date: string, action: () => void, refreshFn?: () => void }
...
function NotificationItem(props: NotificationItemProps) {
    const state = useMemo(() => ({
        ...
        tripId: props.tripId,
        date: props.date,
    }), [props.date, props.id, props.issuerName, props.notificationTypeId, props.tripId])
    ...
    return (
        state.notificationStyle ?
            <View style={{ flexDirection: 'row', width: '100%', flex: 1, padding: 2 }}>
                ...
                <IconButton icon="trash-can-outline" style={{ alignSelf: 'center' }} color="grey" onPress={() => handleDeleteNotification()} />
            </View>
            :
            <></>
    );
