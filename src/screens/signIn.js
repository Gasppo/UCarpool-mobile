...
import { storage, UCA_BLUE, UCA_GREEN } from '../constants';
...
export default function SignInScreen(props) {
  const { authState, login, logout, refresh, loading } = useContext(AuthContext)
  ...
  const fetching = props.authentication.isFetching || loading;
  ...
  React.useEffect(() => {
    if (authState.email && !authState.hasLoggedInOnce) props.navigation.navigate('register_details', { inputEmail: authState.email })
    if (authState.email && authState.hasLoggedInOnce) props.fetchUser({ email: authState.email, accessToken: authState.accessToken })
  }, [authState.email, authState.hasLoggedInOnce])
